...
const ProfilePage = async () => {
  ...
  const orderRes = await dataService.getOrders(user.email);
        ...
        <div className="mt-12 flex flex-col">
          {orderRes.items.map((order) => (
            ...
            >
              ...
              <span className="w-1/4">{format(order.dateCreated)}</span>
              <span className="w-1/4">{order.fulfillmentStatus}</span>
            </Link>
          ))}
        </div>
      </div>
